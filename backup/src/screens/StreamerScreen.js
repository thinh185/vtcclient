...
class StreamerScreen extends Component {
  ...
  onPressCancelStreamer = () => {
    ...
    if (liveStatus === LiveStatus.REGISTER || liveStatus === LiveStatus.ON_LIVE) {
      return Alert.alert('Alert', 'Are you sure to discard your live stream, a lot people is watching right now.', [
        {
          text: 'Cancel',
  ...
  onPressSend = () => {
    ...
    if (message !== '') {
      ...
      SocketUtils.emitSendMessage(this.props.streamOnline.roomName, user._id, message, user.username)
    }
    Keyboard.dismiss()
  }
  ...
  renderGroupInput = () => {
    ...
    if (Platform.OS === 'android') {
      if (!onMessage) {
        return (
          <TouchableOpacity onPress={() => this.setState({ onMessage: true })}>
            <Image source={require('../assets/message.png')} style={styles.iconMessage} />
          </TouchableOpacity>
        )
      }
      return (
        <RowContainer flexDirection="row" justifyContent="space-between">
          <SInput
            ...
            autoCorrect={false}
          />
          <TouchableOpacity onPress={this.onPressSend} activeOpacity={0.6}>
            <Image source={require('../assets/ico_send.png')} />
          </TouchableOpacity>
          <TouchableOpacity
            onPress={this.onPressHeart}
            activeOpacity={0.6}
            onLongPress={() => {
              this.setState({ onLongPress: true })
            }}
          >
            <Image source={require('../assets/ico_heart.png')} />
          </TouchableOpacity>
        </RowContainer>
      )
    }
  ...
  renderCancelStreamerButton = () => {
    ...
    return (
      ...
      >
        <Text style={styles.statusLiveStream}>{liveStatus === LiveStatus.ON_LIVE ? 'Kết thúc' : 'Bắt đầu'}</Text>
      </TouchableOpacity>
    )
  }
  ...
  renderSwitchCamera = () => {
    return (
      <TouchableOpacity onPress={() => this.switchCamera()}>
        <Image source={require('../assets/switch_camera.png')} style={styles.iconSwitch} />
      </TouchableOpacity>
    )
  }
  ...
  renderLiveText = () => {
    const { liveStatus } = this.state
    return (
      <SLiveStatusContainer liveStatus={liveStatus}>
  ...
  renderListMessages = () => {
    ...
    return (
      <TouchableWithoutFeedback onPressIn={this.displayOnfocus} onPressOut={this.hideMessage}>
        <Animated.View style={{ opacity: this.state.opacityMessage }}>
          <SAnimatedVIew>
            <FlatList
              ref={(ref) => {
                this.scrollView = ref
    ...
    return (
      <Container>
        <StartColumnContainer>
          ...
          <STopContainer alignItems="center" justifyContent="flex-start">
            <RowContainer>
              {this.renderLiveText()}
              <SNumeralContainer alignItems="center" justifyContents="flex-start">
                <Image source={require('../assets/ico_view.png')} style={styles.iconView} />
                <SCountViewText>{deltailStream.countViewer || 0}</SCountViewText>
              </SNumeralContainer>
            </RowContainer>
          </STopContainer>
          <NodeCameraView
            ...
            autopreview
          />
          {onLongPress && <ReactionContainer style={styles.iconContainer} />}
          <TouchableWithoutFeedback>
            <SBottomContainer alignItems="center" justifyContent="flex-start">
              <RowContainer alignItems="center" justifyContent="flex-end">
                {liveStatus === LiveStatus.ON_LIVE ? this.renderGroupInput() : null}
                {liveStatus === LiveStatus.ON_LIVE && !this.state.onMessage ? this.renderSwitchCamera() : null}
                {!this.state.onMessage && this.renderCancelStreamerButton()}
              </RowContainer>
            </SBottomContainer>
          </TouchableWithoutFeedback>
          <FloatingReaction count={deltailStream.countUrgy || 0} style={styles.icon} type="Angry" />
          <FloatingReaction count={deltailStream.countHappy || 0} style={styles.icon} type="Laugh" />
          <FloatingReaction count={deltailStream.countWow || 0} style={styles.icon} type="Wow" />
          <FloatingReaction count={deltailStream.countHeart || 0} style={styles.icon} type="Like" />
          <FloatingReaction count={deltailStream.countLike || 0} style={styles.icon} type="ThumpUp" />
          <FloatingReaction count={deltailStream.countSad || 0} style={styles.icon} type="Sad" />
          <FLoatingUser user={deltailStream.newuser} style={styles.icon} />
