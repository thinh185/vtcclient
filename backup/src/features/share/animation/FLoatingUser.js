...
class FloatingUser extends Component {
  ...
  render() {
    ...
    return (
      ...
      >
        {isReady && (
          <AnimatedShape key={user._id} height={height} style={{ right: getRandomNumber(30, 80) }} time="10000">
            <RowContainer alignItems="center">
              <Image source={require('../../assets/avatar_1.png')} style={styles.iconAvatar} />
              <Text style={styles.newUser}>{`${user.username} đang xem live stream`}</Text>
            </RowContainer>
          </AnimatedShape>
        )}
      </View>
